"""
Proactive Risk / Mode Change Baseline Scheduler.

Combines proactive risk monitoring and mode controller transitions (LO -> WARNING -> HI -> RECOVERY)
with EDF-VD deadlines, but uses static/uniform LO task handling (drops/suspends LO tasks in HI mode)
without utility-aware degradation ranking.
"""
...
class ProactiveRiskMCScheduler(BaseScheduler):
    """
    Proactive Risk / Mode Change Baseline Scheduler.
    """
    ...
    def __init__(
        ...
        super().__init__(name="Proactive Risk / Mode Change")
    ...
    def _get_effective_deadline(self, job: Job) -> float:
        ...
        - In HI mode: HI tasks use actual deadline.
    ...
    def select_job(self, current_time: float) -> Optional[Job]:
        ...
        In HI mode, LO tasks are suppressed.
        """
        active_ready = [j for j in self.ready_queue if not j.completed]
        if self.current_mode == SystemMode.HI:
            active_ready = [j for j in active_ready if j.task.criticality == Criticality.HI]
    ...
    def should_preempt(self, current_job: Job, candidate_job: Job) -> bool:
        ...
        if self.current_mode == SystemMode.HI and candidate_job.task.criticality == Criticality.LO:
            return False
